...
<style type="text/css">
	body {
		background: url("login2.jpg") center fixed;
	}
</style>
<body>
...
<?php
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if (!empty($_POST['id'])) {
				$e = mysqli_real_escape_string($dbcon, $_POST['id']);
				} else {
				$e = FALSE;
				echo '<p class="error">You forgot to enter id.</p>';
				}
				...
				{           //if no problems 
							...
							$q = "SELECT * FROM userdetails WHERE (faculty_ID='$e' AND password='$p')";
							...
							{       //if one database row (record) matches the input:-
									...
									$url = ($_SESSION['user_level'] === 1) ? 'admin.php' : 'eventdetail.php';
